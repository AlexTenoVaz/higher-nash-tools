...
def derivative_dictionary(
    ...
    if variables_list is None:
        # por defecto usamos los símbolos globales x,y,z
        # (o menos si number_of_vars<3)
        base_vars = (x, y, z)
...
def jacobian_matrix(
    ...
    derivative_dict = derivative_dictionary(
        f,
        fac=True,
        col_vectors=col_vectors,
        variables_list=variables_list,
        number_of_vars=number_of_vars,
        jac_order_local=jac_order_local
    )
    ...
    for r_idx, row in enumerate(row_vectors):
        for c_idx, col in enumerate(col_vectors):
            ...
            subtracted_vector = tuple(
                c - r for r, c in zip(row, col)
            )

            if all(entry >= 0 for entry in subtracted_vector):
                matching_derivative = derivative_dict.get(
                    subtracted_vector,
                    0
                )
